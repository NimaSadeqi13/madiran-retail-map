...
type Contract = { leaseStart?: string; leaseEnd?: string; owner?: string; rent?: string; increase?: number; area?: string; deposit?: string; address?: string; phone?: string; facilityNote?: string; photo1?: string; photo2?: string; contractNote?: string };
...
const contracts: Record<string, Contract> = {
  "تبریز": { leaseStart: "1405/01/05", leaseEnd: "1408/01/05", owner: "سراج ادیبی موسوی", rent: "167/000/000", increase: 0.25, area: "100", deposit: "500 میلیون تومان", address: "چهارراه شریعتی", phone: "041-51263000", facilityNote: "دارای فضای اداری ویژه شعب سرپرستی", photo1: "/stores/tabriz-1.png", photo2: "/stores/tabriz-2.png" },
  "اصفهان": { leaseStart: "1404/11/15", leaseEnd: "1407/11/15", owner: "شرکت ایرانسل", rent: "100/000/000", increase: 0.30, area: "225", deposit: "یک میلیارد تومان", address: "میدان جمهوری", phone: "031-91550031", facilityNote: "دارای بخش اداری و سالن جلسات مجزای سرپرستی", photo1: "/stores/isfahan-1.jpg", photo2: "/stores/isfahan-2.jpg" },
  ...
  "دزفول": { leaseStart: "1404/06/25", leaseEnd: "1407/06/25", owner: "محمدرضا گوشوار", rent: "55/000/000", increase: 0.25, area: "78", deposit: "500 میلیون تومان", address: "خیابان طالقانی، جنب مسجد عاملی", phone: "061-91013595", photo1: "/stores/dezful-1.jpeg", photo2: "/stores/dezful-2.jpg" },
  ...
  "شهرری": { leaseStart: "1404/12/10", leaseEnd: "1406/12/10", owner: "علی اکبر دانش منش", rent: "110/000/000", increase: 0.25, area: "60", deposit: "یک میلیارد تومان", address: "بلوار امام حسین، روبه‌روی خیابان مصطفی خمینی", phone: "021-41703900", photo1: "/stores/shahr-rey-1.jpg", photo2: "/stores/shahr-rey-2.jpg" },
  "ساوه": { leaseStart: "1404/07/20", leaseEnd: "1407/08/20", owner: "علیرضا رستگار", rent: "90/000/000", increase: 0.25 },
  "کلاردشت": { leaseStart: "1404/09/17", leaseEnd: "1407/09/01", owner: "بیگلریان", rent: "43/000/000", increase: 0.25, area: "60", deposit: "250 میلیون تومان", address: "حسن کیف، بلوار امام خمینی", phone: "011-42726120", photo1: "/stores/kelardasht-1.jpg", photo2: "/stores/kelardasht-2.jpg" },
  "هشتگرد": { leaseStart: "1404/08/30", leaseEnd: "1406/08/30", owner: "توده دهقان", rent: "130/000/000", increase: 0.30, area: "94", deposit: "600 میلیون تومان", address: "بلوار امام خمینی، نرسیده به میدان امام خمینی", phone: "026-34060630", photo1: "/stores/hashtgerd-1.jpg", photo2: "/stores/hashtgerd-2.jpg" },
  "انزلی": { leaseStart: "1404/10/15", leaseEnd: "1407/10/5", owner: "تهمتن", rent: "105/000/000", increase: 0.30, area: "90", deposit: "500 میلیون تومان", address: "خیابان مطهری", phone: "013-44971200", photo1: "/stores/anzali-1.jpg", photo2: "/stores/anzali-2.jpg" },
  ...
  "لاهیجان": { leaseStart: "1404/12/12", leaseEnd: "1407/12/12", owner: "محمدحسین طالب پور", rent: "210/000/000", increase: 0.30, area: "87", deposit: "800 میلیون تومان", address: "روبه‌روی باغ ملی", phone: "013-44971100", photo1: "/stores/lahijan-1.jpg", photo2: "/stores/lahijan-2.png" },
  ...
  "بندرعباس": { leaseStart: "1405/09/15", leaseEnd: "1407/09/15", owner: "احمد عبدلی", rent: "100/000/000", increase: 0.30, area: "86", deposit: "یک میلیارد تومان", address: "خیابان مصطفی خمینی، بین چهارراه اتوبوسرانی و صادقیه، روبه‌روی پست بانک", phone: "076-91010260", photo1: "/stores/bandar-abbas-1.jpg", photo2: "/stores/bandar-abbas-2.jpg" },
  "لنگرود": { leaseStart: "1405/03/01", leaseEnd: "1410/03/01", owner: "رضا ابراهیمی فتیده", rent: "100/000/000", increase: 0.30, area: "100", deposit: "800 میلیون تومان", address: "بلوار امام خمینی، بعد از میدان نماز", phone: "013-44971700", photo1: "/stores/langarud-1.jpeg", photo2: "/stores/langarud-2.jpeg" },
...
export default function Home() {
  ...
  return <main>
    ...
    {selected && <div className="drawer-layer" role="presentation" onMouseDown={(e) => e.target === e.currentTarget && setSelected(null)}><aside className="detail-drawer" role="dialog" aria-modal="true" aria-labelledby="detail-title"><button className="drawer-close" onClick={() => setSelected(null)} aria-label="بستن پنل">×</button><div className="drawer-accent" style={{ background: brandMeta[selected.brand].color }} /><div className="drawer-header"><span className="drawer-kicker">{statusMeta[selected.status]} · سال {selected.year}</span><h2 id="detail-title">{selected.city}</h2><p>{brandMeta[selected.brand].label} / {selected.action}</p></div><div className="photo-pair">{[selected.photo1, selected.photo2].map((photo, i) => photo ? <figure className="store-photo" key={`${selected.id}-${i}`}><img src={photo} alt={`تصویر ${toFa(i + 1)} فروشگاه ${selected.city}`} /><figcaption>{i === 0 ? "نمای فروشگاه" : "فضای پروژه"}</figcaption></figure> : <div className="photo-placeholder" key={`${selected.id}-${i}`}><span className="photo-icon">⌁</span><b>عکس {toFa(i + 1)}</b><small>{i === 0 ? "تصویر نمای فروشگاه" : "تصویر فضای داخلی"}</small></div>)}</div><div className="drawer-section-title"><span>اطلاعات قرارداد ملکی</span><small>برگرفته از اکسل و پروپوزال توسعه</small></div><div className="detail-fields"><div><span>شروع اجاره</span><b>{selected.leaseStart ? toFa(selected.leaseStart) : "ثبت نشده"}</b></div><div><span>پایان اجاره</span><b>{selected.leaseEnd ? toFa(selected.leaseEnd) : "ثبت نشده"}</b></div><div><span>مالک / مالکین</span><b>{selected.owner || "ثبت نشده"}</b></div><div><span>مبلغ اجاره خالص</span><b>{selected.rent ? `${toFa(selected.rent)} تومان` : "ثبت نشده"}</b></div><div><span>شیب افزایشی</span><b>{selected.increase !== undefined ? `${toFa(Math.round(selected.increase * 100))}٪` : "ثبت نشده"}</b></div><div><span>متراژ فروشگاه</span><b>{selected.area ? `${toFa(selected.area)} متر مربع` : "ثبت نشده"}</b></div><div><span>مبلغ ودیعه</span><b>{selected.deposit ? toFa(selected.deposit) : "ثبت نشده"}</b></div><div><span>هزینه اجرای پروژه</span><b>ثبت نشده</b></div><div><span>نوع پروژه</span><b>{selected.action}</b></div><div><span>آدرس شعبه</span><b>{selected.address || "ثبت نشده"}</b></div><div><span>شماره تماس</span><b>{selected.phone ? toFa(selected.phone) : "ثبت نشده"}</b></div></div>{selected.facilityNote && <div className="facility-note"><b>امکانات پروژه</b><p>{selected.facilityNote}</p></div>}<div className="point-message" style={{ "--drawer-color": brandMeta[selected.brand].color } as React.CSSProperties}><span>نقطه فروش</span><p>{selected.pointNote}</p></div>{selected.verification && <div className="verification"><b>نیازمند بررسی</b><p>{selected.verification}</p></div>}{selected.contractNote && <div className="verification"><b>یادداشت اطلاعات قرارداد</b><p>{selected.contractNote}</p></div>}<button className="drawer-action" style={{ background: brandMeta[selected.brand].color }} onClick={() => setSelected(null)}>بازگشت به نقشه</button></aside></div>}
  </main>;
}
